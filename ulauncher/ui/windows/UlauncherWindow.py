...
from gi.repository import Gtk, Gdk, GLib, Keybinder  # type: ignore
...
class UlauncherWindow(Gtk.Window, WindowHelper):
    ...
    def on_input_key_press_event(self, widget, event):
        ...
        if self.results_nav:
            if ctrl and keyname in ('Up', 'ISO_Left_Tab', 'k') or (ctrl and keyname == 'p'):
                self.results_nav.go_up()
                return True
            if ctrl and keyname in ('j', 'Down', 'Tab') or (ctrl and keyname == 'n'):
            ...
            elif ctrl and keyname.isdigit() and 0 < int(keyname) < 10:
                # on Alt+<num>
                try:
